...
class Sequence(nn.Module): 
    def __init__(self, num_classes, seq_length, embedding_dimension=100, num_layers=3, hidden_size_lstm=256):
        ...
        # Define a list to hold the LSTM layers
        self.lstms = nn.ModuleList()
        for i in range(num_layers):
            input_size = self.embedding_dimension if i == 0 else hidden_size_lstm * 2  # *2 for bidirectional
    ...
    def forward(self, input):
        ...
        # Concatenate the final hidden states from both directions of the last LSTM layer
